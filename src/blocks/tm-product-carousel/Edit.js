...
class Edit extends Component {
		...
		return (
			<div className={`${layout === 1 ? 'wprig-post-list-img' : 'wprig-post-grid-img'} wprig-post-img wprig-post-img-${imageAnimation}`}>
				<img className="wprig-post-image" src={post.wprig_featured_image_url && post.wprig_featured_image_url[imgSize][0]} />
				{
					...
					<div className={`wprig-tm-product-carousel-cat-position wprig-tm-product-carousel-cat-position-${categoryPosition}`}>
						<span className="wprig-tm-product-carousel-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />
					</div>
				}
			</div>
		)
	...
	renderCardContent = (post) => {
		...
		let title = <h3 className="wprig-tm-product-carousel-title"><a>{post.title.rendered}</a></h3>
		return (
			<div className={`${layout === 1 ? 'wprig-post-list-content' : 'wprig-post-grid-content'}`}>
				{(showCategory === 'default') && <span className="wprig-tm-product-carousel-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />}
				...
				<div className="wprig-tm-product-carousel-meta">
				...
				{showcartButton && <div className="wprig-tm-product-carousel-btn-wrapper"><a className={`wprig-tm-product-carousel-btn wprig-button-${cartButtonStyle} is-${cartButtonSize}`}>{cartButtonText}</a></div>}
			</div>
		)
	}
	...
	loadCarousel(){
		...
		if(posts && posts.length > 0){
			...
			if(jQuery(".wprig-block-"+uniqueId).find(".wprig-tm-product-carousel-wrapper").hasClass("slick-initialized")){
					jQuery(".wprig-block-"+uniqueId).find(".wprig-tm-product-carousel-wrapper").slick('unslick').slick("reinit");
			}else{				
				var carouselParams = this.props.carouselParams;
				setTimeout(function(){				
					jQuery(".wprig-block-"+uniqueId).find(".wprig-tm-product-carousel-wrapper").slick(carouselParams);
				},500,this)
			}
	...
	renderCarouselContent(posts){
		...
		return posts.map(post => {
			if (post) {
				return (
					<div className={`wprig-tm-product-carousel ${layout === 1 ? 'wprig-post-list-view' : 'wprig-post-grid-view'} wprig-tm-product-carousel-style-${style}`}>
						...
						(posts && posts.length) ?
							<Fragment>
								<div
									...
									className={`wprig-tm-product-carousel-wrapper wprig-tm-product-carousel-layout-${layout} ${(layout === 2) ? 'wprig-tm-product-carousel-column wprig-tm-product-carousel-column-md' + column.md + ' ' + 'wprig-tm-product-carousel-column-sm' + column.sm + ' ' + 'wprig-tm-product-carousel-column-xs' + column.xs : ''}`}>
							...
							:
							<div className="wprig-tm-product-carousel-is-loading">
								<Spinner />
							</div>
					} 
				</div>
