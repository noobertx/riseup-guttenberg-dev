...
class Edit extends Component {
		...
		return (
			<div className={`${layout === 1 ? 'wprig-post-list-img' : 'wprig-post-grid-img'} wprig-post-img wprig-post-img-${imageAnimation}`}>
				<img className="wprig-post-image" src={post.wprig_featured_image_url && post.wprig_featured_image_url[imgSize][0]} />
				{
					...
					<div className={`wprig-productgrid-cat-position wprig-productgrid-cat-position-${categoryPosition}`}>
						<span className="wprig-productgrid-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />
					</div>
				}
			</div>
		)
	...
	renderCardContent = (post) => {
		...
		let title = <h3 className="wprig-productgrid-title"><a>{post.title.rendered}</a></h3>

		return (
			<div className={`${layout === 1 ? 'wprig-post-list-content' : 'wprig-post-grid-content'}`}>
				{(showCategory === 'default') && <span className="wprig-productgrid-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />}
				...
				{showTitle && (titlePosition == true) && title}
				{
					...
					// <div className="wprig-productgrid-meta">
				...
				{showExcerpt && <div className="wprig-productgrid-intro" dangerouslySetInnerHTML={{ __html: this.truncate(post.excerpt.rendered, excerptLimit) }} />}
				{showReadMore && <div className="wprig-productgrid-btn-wrapper"><a className={`wprig-productgrid-btn wprig-button-${readmoreStyle} is-${readmoreSize}`}>{buttonText}</a></div>}
			</div>
		)
	}
						...
						(posts && posts.length) ?
							<Fragment>
								<div
									...
									className={`wprig-productgrid-wrapper wprig-productgrid-layout-${layout} ${(layout === 2) ? 'wprig-productgrid-column wprig-productgrid-column-md' + column.md + ' ' + 'wprig-productgrid-column-sm' + column.sm + ' ' + 'wprig-productgrid-column-xs' + column.xs : ''}`}>
									{
										posts && posts.map(post => {
											if (post) {
												return (
													<div className={`wprig-productgrid ${layout === 1 ? 'wprig-post-list-view' : 'wprig-post-grid-view'} wprig-productgrid-style-${style}`}>
								...
								</div>
								{
									...
									<div className="wprig-productgrid-pagination">
										{
							...
							:
							<div className="wprig-productgrid-is-loading">
								<Spinner />
							</div>
					} 
				</div>
