            ...
            {separatorStyle &&
                <Fragment>
                    {separators[separatorStyle].type == 'css' &&
                        <span className={`yani-separator-type-css yani-separator-${separatorStyle}`}></span>
                    }
                    {separators[separatorStyle].type == 'svg' &&
                        <span className={`yani-separator-type-svg yani-separator-${separatorStyle}`}>{separators[separatorStyle].svg}</span>
                    }
                </Fragment>
            }
        </Fragment>
        ...
        return (
            <Fragment>
                ...
                <div className={`yani-block-${uniqueId}${className ? ` ${className}` : ''}`}>
                    <div
                        className={`yani-block-info-box yani-info-box-layout-${layout}`}
                        onContextMenu={event => handleContextMenu(event, this.wprigContextMenu.current)}
                    >
                        {(layout != 4 && mediaType) &&
                            <div className={`yani-info-box-media ${(useMediaBg && mediaType !== 'image') ? ' yani-media-has-bg' : ''}`} onClick={() => this.handlePanelOpenings('Media')}>
                                {(mediaType == 'icon' && iconName) &&
                                    <i className={"yani-info-box-icon " + iconName} />
                                }
                                {(mediaType == 'image') &&
                                    <Fragment>
                                        {
                                            (imageType === 'local' && image.url != undefined) ?
                                                <img className="yani-info-box-image" src={image.url} srcset={image2x.url != undefined ? image.url + ' 1x, ' + image2x.url + ' 2x' : ''} alt={imgAlt && imgAlt} />
                                                :
                                                (imageType === 'external' && externalImageUrl.url != undefined) ?
                                                    <img className="yani-info-box-image" src={externalImageUrl.url} alt={imgAlt && imgAlt} />
                                                    :
                                                    <div className="yani-info-box-image yani-image-placeholder"><i className="far fa-image" /></div>
                                        }
                                    </Fragment>
                                }
                                {(mediaType == 'number' && number) &&
                                    <span className="yani-info-box-number">{number}</span>
                                }
                            </div>
                        }
                        <div className="yani-info-box-body">
                            <div className={`yani-info-box-title-container ${separatorStyle ? 'yani-has-separator' : ''} ${separatorPosition ? 'yani-separator-position-' + separatorPosition : ''}`} >
                            {enableTitle == 1 &&
                                <div className="yani-info-box-title-inner">
                                    {separatorStyle && (separatorPosition == 'left' || separatorPosition == 'top' || separatorPosition == 'leftright') ? <div className="yani-separator yani-separator-before">{renderSeparators}</div> : ''}
                                    ...
                                    <div onClick={() => this.handlePanelOpenings('Title')}>
                                        <RichText
                                            ...
                                            className={`yani-info-box-title text-${alignment} ${titleColor}`}
                                            keepPlaceholderOnFocus
                                            placeholder={__('Add Text...')}
                                            onChange={value => setAttributes({ title: value })}
                                            value={title} />
                                    </div>
                                    {separatorStyle != '' && (separatorPosition == 'right' || separatorPosition == 'bottom' || separatorPosition == 'leftright') ? <div className="yani-separator yani-separator-after">{renderSeparators}</div> : ''}
                                </div>

                                }

                                {enableSubTitle == 1 &&
                                    <div className="yani-info-box-sub-title-container" onClick={() => this.handlePanelOpenings('Sub Title')}>
                                        <RichText
                                            key="editable"
                                            tagName={subTitleTagName}
                                            className={`yani-info-box-sub-title text-${alignment} ${subTitleColor}`}
                                            keepPlaceholderOnFocus
                                            placeholder={__('Add Text...')}
                            ...
                            {
                                enableContent &&
                                <div className="yani-info-box-content" onClick={() => this.handlePanelOpenings('Content')}>
                                    <RichText
                                        key="editable"
                                        tagName='div'
                                        className={`yani-info-box-text text-${alignment} ${textfieldColor}`}
                                        keepPlaceholderOnFocus
                                        placeholder={__('Add Text...')}
