...
class Save extends Component {
	...
	render() {
		...
		return (
			<div className={`wprig-grids wprig-section wprig-grids wprig-block-${uniqueId} ${(rowBg.bgimgParallax && rowBg.bgimgParallax == 'animated') ? 'wprig-section-parallax' : ''}`} {...rowId ? { id: rowId } : ''} {...animationAttr(animation)}>
				...
				<div className={this.getClassName()}>
					<div className={`wprig-grids ${(heightOptions == 'window') ? 'wprig-row-height-window' : ''}`}>
						<InnerBlocks.Content />
					</div>
				</div>
			</div>
