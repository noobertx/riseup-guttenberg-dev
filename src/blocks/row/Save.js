...
class Save extends Component {
	...
	render() {
		const { attributes: { uniqueId, animation, align, rowContainerWidth, position, rowId, rowBg, shapeTop, shapeBottom, heightOptions } } = this.props
		...
		return (
			<div className={`wprig-section wprig-block-${uniqueId} ${(rowBg.bgimgParallax && rowBg.bgimgParallax == 'animated') ? 'wprig-section-parallax' : ''}`} {...rowId ? { id: rowId } : ''} {...animationAttr(animation)}>
				...
				<div className={this.getClassName()}>
					<div className={`wprig-row ${position} ${(heightOptions == 'window') ? 'wprig-row-height-window' : ''}`}>
						<InnerBlocks.Content />
					</div>
				</div>
			</div>
