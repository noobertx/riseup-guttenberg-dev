...
class Edit extends Component {
		...
		return (
			<div className={`${layout === 1 ? 'wprig-post-list-img' : 'wprig-post-grid-img'} wprig-post-img wprig-post-img-${imageAnimation}`}>
				<img className="wprig-post-image" src={post.wprig_featured_image_url && post.wprig_featured_image_url[imgSize][0]} />
				{
					...
					<div className={`wprig-product-carousel-cat-position wprig-product-carousel-cat-position-${categoryPosition}`}>
						<span className="wprig-product-carousel-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />
					</div>
				}
			</div>
		)
	...
	renderCardContent = (post) => {
		...
		let title = <h3 className="wprig-product-carousel-title"><a>{post.title.rendered}</a></h3>
		return (
			<div className={`${layout === 1 ? 'wprig-post-list-content' : 'wprig-post-grid-content'}`}>
				{(showCategory === 'default') && <span className="wprig-product-carousel-category wprig-backend" dangerouslySetInnerHTML={{ __html: post.wprig_category }} />}
				...
				<div className="wprig-product-carousel-meta">
				...
				{showcartButton && <div className="wprig-product-carousel-btn-wrapper"><a className={`wprig-product-carousel-btn wprig-button-${cartButtonStyle} is-${cartButtonSize}`}>{cartButtonText}</a></div>}
			</div>
		)
	}
	...
	renderCarouselContent(posts){
		...
		return posts.map(post => {
			if (post) {
				return (
					<div className={`wprig-product-carousel ${layout === 1 ? 'wprig-post-list-view' : 'wprig-post-grid-view'} wprig-product-carousel-style-${style}`}>
						...
						(posts && posts.length) ?
							<Fragment>
								<div
									...
									className={`wprig-product-carousel-wrapper wprig-product-carousel-layout-${layout} ${(layout === 2) ? 'wprig-product-carousel-column wprig-product-carousel-column-md' + column.md + ' ' + 'wprig-product-carousel-column-sm' + column.sm + ' ' + 'wprig-product-carousel-column-xs' + column.xs : ''}`}>
							...
							:
							<div className="wprig-product-carousel-is-loading">
								<Spinner />
							</div>
					} 
				</div>
