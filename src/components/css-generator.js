...
const objectField = (data) => {
    ...
    } else if (data.openColorSVG) {
        return { data: cssColorSVG(data), action: 'append' }; //Color Advanced
...
export const CssGenerator = (settings, blockName, blockID, isInline = false, global = false, frontend = false, globalAttributes = undefined) => {
    ...
    Object.keys(settings).forEach((key) => {
        let attributes = [];
        if (global === false) {
            attributes = typeof blockName === 'string' ? wp.blocks.getBlockType('yani/' + blockName).attributes : blockName
        } else {
...
const setStyle = (styleCss, blockID, global = false, frontend = false) => {
    let styleSelector = window.document, id = 'rsup-block-' + blockID;
    if (global && frontend) {
        id = 'rsup-global-styles';
    }
