...
export const cssTypography = (v) => {
    ...
    //         font = "@import url('https://fonts.googleapis.com/css?family=" + v.family.replace(/\s/g, '+') + ':' + (v.weight || 400) + "&display=swap');"
    //     }
    // }
    let data = { md: [], sm: [], xs: [] }
...
export const cssColorSVG = (v) => {
    let data = "";
    data += (v.color ? 'fill: ' + v.color + ';' : '')
    data += (v.width ? 'width: ' + v.width.val+ v.width.unit+ ';' : '')
    data += (v.height ? 'height: ' + v.height.val+ v.height.unit+ ';' : '')
    return ' svg{' + data + '}';
}
