...
export const globalAttributes = {
    ...
    selectPosition: {
        type: 'string',
        default: 'relative',
        style: [{ selector: '{{WPRIG}} {position:{{selectPosition}};}' }]
    },
    positionXaxis: {
        type: 'object',
        default: {
        ...
        style: [{ selector: '{{WPRIG}} {left:{{positionXaxis}};}' }]
    },
    positionYaxis: {
        type: 'object',
        default: {
        ...
        style: [{ selector: '{{WPRIG}} {top:{{positionYaxis}};}' }]
    },
    globalZindex: {
        type: 'string',
        default: '0',
        style: [{ selector: '{{WPRIG}} {z-index:{{globalZindex}};}' }]
    },
    hideTablet: {
        type: 'boolean',
        default: false,
        style: [{ selector: '@media (min-width: 992px){{{WPRIG}}{display:none;}}' }]
    },
    hideMobile: {
        type: 'boolean',
        default: false,
        style: [{ selector: '{{WPRIG}}{display:none;}' }]
    },
    globalCss: {
        type: 'string',
        default: '',
...
export function globalSettingsPanel(enablePosition, selectPosition, positionXaxis, positionYaxis, globalZindex, hideTablet, hideMobile, globalCss, setAttributes, isReturn = false) {
    ...
    const Output = <Fragment>
        ...
        <TextareaControl
            value={globalCss}
            rows={5}
            placeholder={__('Use {{WPRIG}} before the selector to wrap element. Otherwise it works globally.')}
